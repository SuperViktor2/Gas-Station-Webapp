    ...
    <body id="body">
      ...
      <div class="container">
        ...
        <div class="info">
            ...
            <?php
                ...
                $sql = "SELECT p.idPumpa, p.naziv, m.idMjesto, vrstaGoriva.Naziv, Gorivo.cijena, Gorivo.status from Pumpa p 
      ...
      <div class="forma">
            <form name="gorivo" id="forma" method="get" action="<?php echo $_SERVER['PHP_SELF']; ?>">
                ...
                <input id="id" class="txtBox" name="id" type="number" placeholder="Unesite ID pumpe koju Želite pretražiti"/><br>

                <?php

                    $id = $_GET['id'];

                    $sql2 = "SELECT p.idPumpa, p.naziv, m.idMjesto, vrstaGoriva.Naziv, Gorivo.cijena, Gorivo.status from Pumpa p 
                        LEFT JOIN Mjesto m ON p.idPumpa = m.idPumpa 
                        LEFT JOIN Gorivo on m.idMjesto = Gorivo.idMjesto 
                        LEFT JOIN vrstaGoriva on Gorivo.idvrstaGoriva = vrstaGoriva.idVrste WHERE p.idPumpa = '$id'";
                    $odg2 = $veza->selectDB($sql2);

                    echo "<ul style='list-style: none; float: left; text-align: left; color: white;'>";
                    while ($red2 = $odg2->fetch_assoc()) {
                        echo "<li>" . $red2['naziv'] . ', ' . $red2['idMjesto'] . ', ' .$red2['Naziv'] . ', ' . $red2['cijena'] . ', ' . $red2['status'];
                    }
                    echo "</ul>";

                ?>
            </form>
            <input name="submit" class="gumb" type="submit" value="Pretraži" form="forma"/><br>
        </div>
